...
public:
	...
	/**
	 * Collection receiving SeasonPhase — 0..1 around the year, 0 at the winter
	 * solstice. Materials read it instead of carrying their own frozen phase, so
	 * anything seasonal turns with the same calendar the sun and moon already use.
	 *
	 * Leave unset to skip the write entirely.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Astronomy|Season")
	TObjectPtr<class UMaterialParameterCollection> SeasonCollection;

	/** Scalar written into SeasonCollection. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = "Astronomy|Season")
	FName SeasonPhaseParameter = TEXT("SeasonPhase");

	/** Phase 0..1 around the year, 0 at the winter solstice. */
	UFUNCTION(BlueprintPure, Category = "Astronomy|Season")
	float GetSeasonPhase() const;
	...
	/** Yaw offset (deg) added to every celestial azimuth (sun, moon, stars) so the astronomical
	 *  compass — 0° = geographic North, which the engine maps to world +X — lines up with THIS
	 *  world's sun-anchored compass (North = +Y, East = -X). 90 turns the whole sky clockwise 90°,
	 *  putting sunrise in world East (-X). Rotates the bodies only, not the actor. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Astronomy|Location",
		meta = (ClampMin = "-360.0", ClampMax = "360.0"))
	float CelestialNorthYaw = 90.0f;
...
private:
	...
	/** Publishes SeasonPhase to SeasonCollection for season-driven materials. */
	void UpdateSeason();
